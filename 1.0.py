...
font_scoreDisplay = pygame.font.SysFont("隶书", 30)
game_over = font_big.render("游戏结束", True, BLACK)
scoreDisplay = font_scoreDisplay.render("当前分数", True, BLACK)
# 定义分数
SCORE = 0
# 敌方车辆的速度
SPEED = 5
...
class Enemy(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if self.rect.top > height:
            ...
            self.rect.left = random.randint(0, 250)


# 定义敌人类, 继承sprite
class Enemy2(pygame.sprite.Sprite):
    def __init__(self):
        super(Enemy2, self).__init__()  # 调用父类的init初始化方法
        x, y = (random.randint(24, 376), 0)
        self.image = pygame.image.load("./resource/Enemy.png")  # 返回底图层
        self.surf = pygame.Surface((35, 70))
        self.rect = self.surf.get_rect(center=(x, y))
    ...
    def move(self):
        ...
        if self.rect.top > height:
            SCORE += 1
            self.rect.top = 0  # 敌人到达底部后，又重新回到顶部
            self.rect.left = random.randint(150, 376)
